                    ...
                    <form action="{{ route('hospital.update', $hospital->id) }}" method="post" enctype="multipart/form-data">
                        ...
                        <div class="form-row">
                            <div class="form-group col-md-6 ">
                                ...
                                <input type="text" placeholder="{{ ___('placeholder.Name') }}" class="form-control input-style-1" name="name" value="{{ old('name', $hospital->name) }}">
                                @error('name')
                            ...
                            <div class="col-md-6">
                                    ...
                                    <select name="type" class="form-control input-style-1 select2">
                                        @foreach (HospitalType::cases() as $type)
                                            <option value="{{ $type->value }}" @selected(old('type', $hospital->type?->value ?? null) == $type->value)>
                                                {{ $type->name }}
                                            </option>
                                        @endforeach
                                    </select>
                            ...
                            <div class="form-group col-md-6 ">
                                ...
                                <input type="text" class="form-control input-style-1 " name="registration_no" placeholder="{{ ___('placeholder.registration_no') }}" value="{{ old('registration_no', $hospital->registration_no) }}">
                                @error('text') <p class="pt-2 text-danger">{{ $message }}</p> @enderror
                            </div>
                            <div class="form-group col-md-6 ">
                                <label>{{ ___('label.email') }}<span class="text-danger">*</span></label>
                                <input type="email" class="form-control input-style-1 " name="email" placeholder="{{ ___('placeholder.email') }}" value="{{ old('email', $hospital->email) }}">
                                @error('email') <p class="pt-2 text-danger">{{ $message }}</p> @enderror
                            </div>
                            <div class="form-group col-md-6 ">
                                <label>{{ ___('label.phone') }}<span class="text-danger">*</span></label>
                                <input type="text" class="form-control input-style-1 " name="phone" placeholder="{{ ___('placeholder.phone') }}" value="{{ old('phone', $hospital->phone) }}">
                                @error('phone') <p class="pt-2 text-danger">{{ $message }}</p> @enderror
                            </div>
                            <div class="form-group col-md-6 ">
                                <label>{{ ___('label.address') }}<span class="text-danger">*</span></label>
                                <input type="text" class="form-control input-style-1 " name="address" placeholder="{{ ___('placeholder.address') }}" value="{{ old('address', $hospital->address) }}">
                                @error('address') <p class="pt-2 text-danger">{{ $message }}</p> @enderror
                            </div>
                            <div class="form-group col-md-6 ">
                                <label>{{ ___('label.district') }}<span class="text-danger">*</span></label>
                                <input type="text" class="form-control input-style-1 " name="district" placeholder="{{ ___('placeholder.district') }}" value="{{ old('district', $hospital->district) }}">
                                @error('district') <p class="pt-2 text-danger">{{ $message }}</p> @enderror
                            </div>
                            <div class="form-group col-md-6 ">
                                <label>{{ ___('label.latitude') }}</label>
                                <input type="number" class="form-control input-style-1 " name="latitude" placeholder="{{ ___('placeholder.latitude') }}" value="{{ old('latitude', $hospital->latitude) }}">
                                @error('latitude') <p class="pt-2 text-danger">{{ $message }}</p> @enderror
                            </div>
                            <div class="form-group col-md-6 ">
                                <label>{{ ___('label.longitude') }}</label>
                                <input type="number" class="form-control input-style-1 " name="longitude" placeholder="{{ ___('placeholder.longitude') }}" value="{{ old('longitude', $hospital->longitude) }}">
                                @error('longitude') <p class="pt-2 text-danger">{{ $message }}</p> @enderror
                            </div>
                            <div class="form-group col-md-6 ">
                                <label>{{ ___('label.website_url') }}<span class="text-danger">*</span></label>
                                <input type="url" class="form-control input-style-1 " name="website_url" placeholder="{{ ___('placeholder.website_url') }}" value="{{ old('website_url', $hospital->website_url) }}">
                              ...
                              <div class="col-md-6">
                                    ...
                                    <select name="status" class="form-control input-style-1 select2">
                                        @foreach (Status::cases() as $status)
                                            <option value="{{ $status->value }}" @selected(old('status', $hospital->status?->value ?? null) == $status->value) >
                                                {{ $status->name }}
                                            </option>
                                        @endforeach
                                    </select>
