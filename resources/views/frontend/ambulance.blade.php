    ...
    <section class="pt-16 pb-16 lg:pb-40 bg-white relative z-[1]">
        ...
        <div class="px-4 lg:px-[80px] ">
            ...
            <div class="grid md:grid-cols-2 lg:grid-cols-5 gap-x-6 gap-y-20 justify-center items-center">
                ...
                @foreach ( $ambulances as $ambulance)
                    ...
                    <div
                        ...
                        <div
                            ...
                            <img src="{{ asset('frontend/assets/img/icons-ambulance.png') }}"
                                class="w-full h-full rounded-full object-cover" alt="">
                        </div>

                        <h2 class="text-red-600 font-semibold text-lg mt-18">{{ $ambulance->service_name }}</h2>
                        <h3 class="text-green-600 font-bold text-xl">Driver Name:{{ $ambulance->driver_name }} </h3>
                        <p class="text-gray-700 mt-2">
                            {{ $ambulance->address }}<br>
                            {{ $ambulance->phone }} <br>
                            {{ $ambulance->email }}
                        </p>

                
                        <p class="text-green-600 font-semibold mb-6">registration no: {{ $ambulance->registration_no }}</p>
                        <p class="text-green-600 font-semibold mb-6">ambulance type: {{ $ambulance->ambulance_type }}</p>

                        <a href="tel:{{ $ambulance->phone }}" class="btn-primary rounded-lg btn-black h-10">Call Now <i
                ...
                @if ($ambulances)
                    <div class="mt-3">
                        <x-paginate-show :items="$ambulances" />
                    </div>
                @endif
            </div>
        </div>
